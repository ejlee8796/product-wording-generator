...
<body>
<h3 class="container"> 정제어 INSERT 리스트 </h3>
	<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
	<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
	<div class="container" id="wordList">
		<form action ="${pageContext.request.contextPath}/allInsertWording" method="post" id="allInsertWording" >
			<div id="hidden"></div>
			<div class="panel-body" id="wordBody"></div>
			<input type="submit" value="ALL+INSERT" class="text-center btn btn-warning" />
		</form>
		<div class="text-center listBtn" id="listBtn"></div>
	</div>
	<script>
	$(function(){
		list("goodsWordingInsert", 0);
		...
		$(document).on("click", "#wordBody a", function(){
			...
			$.ajax({
				...
				success : function(data) {
					$("#wording"+id).html("");
					$("#btn"+id).html("");
					var btn = $("<input type='button' value='INSERT' name='"+id+"' class='btn btn-default' />");
					$("#wording"+id).val(data);
					$("#btn"+id).append(btn);
				},
		...
		$(document).on("click", "#wordList input[type=button]", function(){
			var id = $(this).attr("name");
			var word1 = $("#wording"+id).val();
			var word2 = $("#word"+id).val();
			
			$.ajax({
				url : "${pageContext.request.contextPath}/insertWording", type : "get", dataType : "text",
		...
		$.ajax({
			...
			success : function(data) {
				...
				if(startpage >= bottomlimit) {
					prev = $("<li>").html("<a href='${pageContext.request.contextPath}/list?pstartno="+((startpage-2)*onepagelimit)+"' class='prev'>[이전]</a>");
					ul.append(prev);
				}
				...
				for(var i= startpage; i<=endpage; i++) {
					var li = "";
					if(currentpage == i) {
						li = $("<li class='active'>").html("<a href='${pageContext.request.contextPath}/list?pstartno="+(i-1)*onepagelimit+"' id='button' class='btn btn-default'>"+i+"</a>");
					} else {
						li = $("<li>").html("<a href='${pageContext.request.contextPath}/list?pstartno="+(i-1)*onepagelimit+"' id='button' class='btn btn-default'>"+i+"</a>");
					}
					ul.append(li);
				}

				if(pagetotal> endpage ) {
					next = $("<li>").html("<a href='${pageContext.request.contextPath}/list?pstartno="+(endpage*onepagelimit)+"' class='next'>[다음]</a>");
					ul.append(next);
				}
